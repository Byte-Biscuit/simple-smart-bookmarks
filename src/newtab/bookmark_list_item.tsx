...
const BookmarkItemMeta: React.FC<{
    bookmarkInspection?: BookmarkInspection
            ...
            <List.Item.Meta
                ...
                title={
                    bookmarkInspection.note != null ? (
                        <Tooltip title={bookmarkInspection.note}>
                ...
                description={
                    <>
                        {bookmarkInspection.keywords != null ? (
                            <Tooltip
                                title={bookmarkInspection.keywords}
                                placement="topLeft"
                                overlayInnerStyle={{ maxWidth: "800px" }}
                                overlayStyle={{ maxWidth: "none" }}>
                                <p className={"mt-0.5 mb-0 line-clamp-3"}>
                                    {bookmarkInspection.description ||
                                        bookmark.url}
                                </p>
                            </Tooltip>
                        ) : (
                            <p className={"mt-0.5 mb-0 line-clamp-3"}>
                                {bookmarkInspection.description || bookmark.url}
                            </p>
                        )}
                        <p className={"mt-0.5 mb-0 text-blue-500"}>
                            {intl.get("bookmark-added-date").d("Update")}:
                            {dayjs(bookmark.dateAdded).format("YYYY-MM-DD")}
                        </p>
                    </>
                }
            />
...
const BookmarkItem: React.FC<{
    ...
    const { setBookmarkNum } = useContext(NewtabContext)
    const { bookmark, bookmarkList, setBookmarkList } = props
    const [messageApi, contextHolder] = message.useMessage()
    const [bookmarkInspection, setBookmarkInspection] =
        useState<BookmarkInspection>(null)
    ...
    return (
        <>
            ...
            <Modal
                title={intl
                    .get("bookmark-note-update-title")
                    .d("Update Bookmark Note")}
                okText={intl
                    .get("bookmark-note-update-button-ok-text")
                    .d("Submit")}
                cancelText={intl
                    .get("bookmark-note-update-button-cancel-text")
                    .d("Cancel")}
                ...
                onOk={() => {
                    ...
                    if (note != null && note.trim().length !== 0) {
                        ...
                        setCache(key, newObject).then(() => {
                            ...
                            messageApi.info(
                                intl
                                    .get("bookmark-note-update-success")
                                    .d("Update successfully")
                            )
                        })
                    }
                }}
            ...
            <List.Item
                className={classNames({
                    "bg-red-100":
                        bookmarkInspection != null &&
                        bookmarkInspection.status >= 400
                })}
                actions={[
                    <Button
                        className={"px-1"}
                    ...
                    <Button
                        ...
                        onClick={() => {
                            bookmarkInspect(bookmark.url).then(
                                (inspectResult) => {
                                    let key = hashCode(bookmark.url)
                                    getCache(key).then((res) => {
                                        let cacheInspect =
                                            res as unknown as BookmarkInspection
                                        if (res == null) {
                                            setCache(key, inspectResult).then(
                                                () => {
                                                    setBookmarkInspection(
                                                        inspectResult
                                                    )
                                                }
                                            )
                        ...
                        {intl
                            .get("bookmark-inspection-update-text")
                            .d("Update")}
                    </Button>,
                    <Popconfirm
                        title={intl
                            .get("bookmark-remove-popconfirm-title")
                            .d("Confirmation of bookmark deletion")}
                        description={intl
                            .get("bookmark-remove-popconfirm-description")
                            .d(
                                "Are you sure you want to delete this bookmark, you can't restore it after deletion?"
                            )}
                        onConfirm={() => {
                            chrome.bookmarks.remove(bookmark.id).then(() => {
                                removeCache(hashCode(bookmark.url)).then(() => {
                                    ...
                                    countBookmarks().then((count) => {
                                        setBookmarkNum(count)
                                    })
                                })
                            })
                        }}
                        ...
                        <Button
                            ...
                            icon={<DeleteOutlined />}>
                            {intl
                                .get("bookmark-delete-button-text")
                                .d("Delete")}
                        </Button>
                    </Popconfirm>
                ]}>
